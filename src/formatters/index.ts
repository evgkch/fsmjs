 ...
 * `fsmjs/analysis` — the only traversal used here is `edges`, in schema order. A renderer says
 ...
 *
 * Rows come in schema order (state-major, so `FROM` groups). `fsmjs/debug`'s `rules` writes a
