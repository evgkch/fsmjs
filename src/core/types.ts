...
//
// All three share the shape `(context, payload) => …` and differ only in what they aim at:
// `when` at a decision (this is the partiality — the guard), `with` at the context (the
// algebra: the input folds into the state), `by` at the output (the coalgebra: the state
// unfolds into an observation). `with` sees the context before the move, `by` after it.
//
// Each is optional, and an operation that is not written is its own neutral element: `⊤`
// for `when`, `id` for `with`, the unique map into the empty payload for `by`.
// Sparseness is not a convenience, it is that default made visible.
