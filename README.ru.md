...
| [`@evgkch/fsmjs`](#evgkchfsmjs)                                                   | Класс `StateMachine`, носители, схема, шина, сериализация, асинхронность, граф, JSON       |
...
Выходные события публикуются в `rx` — приёмнике канала [`@evgkch/channeljs`](https://github.com/evgkch/channeljs).
...
Прогон сериализуется теми же парами: запись из `history` (`@evgkch/fsmjs/debug`) — готовые JSON-значения.
...
[`@evgkch/fsmjs-inspector`](https://github.com/evgkch/fsmjs-inspector) — инструмент разработки для машин этой библиотеки: текст правил, фигура переходов, классическая диаграмма и прогон, связанные общей подсветкой. Он читает дамп схемы — [открыть инспектор](https://evgkch.github.io/fsmjs-inspector/) — или подключается к работающей машине одной строкой:
...
Виджеты инспектора подключаются и по отдельности, не поднимая целый инспектор: примеры этого репозитория рисуют ими свои машины ([открыть примеры](https://evgkch.github.io/fsmjs/)).
